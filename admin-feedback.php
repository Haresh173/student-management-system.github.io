<?php
  ...
  $result = mysqli_query($conn, "SELECT * FROM `feedback` LIMIT $offset, $total_records_per_page");

  if(isset($_GET['did'])){

      $sqlD = "DELETE FROM feedback WHERE id= '".$_GET['did']."'";
      if (mysqli_query($conn, $sqlD)) {
        echo "<script>alert('Delete Successfully!');</script>";
        echo "<script>window.location.href='admin-feedback.php?page_no=".$page_no."'</script>";
      } else {
...
<body>
   <header>
    ...
    <div class="sidebar">
      ...
      <a href="admin-feedback.php" class="list-group-item list-group-item-action py-2 ripple active">Manage Feedbacks</a>
    </div>
    <div class="content">
        <div class="box">
                          ...
                          while($row = mysqli_fetch_assoc($result)) {
                            ...
                            // <td>".$row['id']."</td>
                              ...
                              echo " <tr>
                                      ...
                                      <td><a class='btn btn-primary btn-sm' data-toggle='modal' data-target='#infoModal".$row['id']."'>View Comment</a>"; ?>

                                      <a class='btn btn-dark btn-sm' href='admin-feedback.php?did=<?php echo $row['id']?>' onclick="javascript:return confirm('Confirm to delete?')">Delete</a></td>
                                      
                          
                          <?php 
                              $count++;

                              echo '<div class="modal fade infoModal" id="infoModal'.$row['id'].'" tabindex="-1" role="dialog" aria-labelledby="infoModal'.$row['id'].'" aria-hidden="true">
                                    <div class="modal-dialog" role="document">
                                      <div class="modal-content">
                                        <div class="modal-header">
                                          <h5 class="modal-title" id="infoModal'.$row['id'].'">Feedback Message</h5>
